...
local function conditional_hover()
  ...
  local test_cmd = ('zig build test -Dtest-filter="%s"'):format(test_name)
...
local function run_last_test()
  if not vim.g.zig_test_cmd then
    vim.g.zig_test_cmd = 'direction=horizontal size=16 cmd="zig build test"'
