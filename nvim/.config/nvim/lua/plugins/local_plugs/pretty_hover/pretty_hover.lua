...
local fn = vim.fn
local lsp = vim.lsp
local win = require('plugins.configs.local_plugs.pretty_hover.window')
local treesitter = vim.treesitter
local hover = {}

local config_max_width = 0.8
local config_max_height = 0.6
local open_link = 'gx'
local scroll_up = '<C-p>'
local scroll_down = '<C-n>'

--[===========================================================================]
--[---------------------------Utility functions-------------------------------]
--[===========================================================================]

-- local function get_client_by_method(method)
...
-- local function get_max_float_width(percent)
...
local function map_keys(buffer, keys, rhs, modes, opts)
  opts = opts or {}
  opts.nowait = true
  opts.noremap = true
  modes = modes or 'n'
  ...
  for _, mode in ipairs(as_table(modes)) do
    for _, lhs in ipairs(as_table(keys)) do
      api.nvim_buf_set_keymap(buffer, mode, lhs, rhs, opts)
    end
  end
end

local function delete_scroll_map(bufnr)
  api.nvim_buf_del_keymap(bufnr, 'n', scroll_down)
  api.nvim_buf_del_keymap(bufnr, 'n', scroll_up)
end
...
local function win_height_increase(content, percent)
  local increase = 0
  local max_width = get_max_float_width(percent)
  local max_len = get_max_content_length(content)
  local new = {}
...
local function scroll_in_float(bufnr, winid)
  if not api.nvim_win_is_valid(winid) or not api.nvim_buf_is_valid(bufnr) then
    return
  end

  for i, map in ipairs({ scroll_down, scroll_up }) do
    map_keys(bufnr, map, function()
...
--   local new = {}
--   local max_float_width = math.floor(vim.o.columns * config_max_width)
--   local max_content_len = get_max_content_length(content)
--   local max_height = math.floor(vim.o.lines * config_max_height)
...
--     end
--     if line:find('^%-%-%-$') then
--       line = string.rep('-', float_option.width)
--     end
--     if line:find('\\') then
--       line = line:gsub('\\', '')
--     end
...
--   local increase = win_height_increase(new, config_max_width)
...
--   scroll_in_float(curbuf, self.winid)

--   map_keys(self.bufnr, 'q', function()
...
--   map_keys(self.bufnr, open_link, function()
--     self:open_link()
--   end)
...
--     buffer = self.bufnr,
--     callback = function()
--       pcall(delete_scroll_map, curbuf)
--     end,
--   })
-- end
...
--   local clients = get_client_by_method(method)
--   if #clients == 0 then
...
--     if self.bufnr and api.nvim_buf_is_valid(self.bufnr) and self.winid and api.nvim_win_is_valid(self.winid) then
...
--       local truncate = string.rep('-', win_conf.width)
...
--     vim.notify('[lspsaga] please install markdown and markdown_inline parser in nvim-treesitter', vim.log.levels.WARN)
--     return
--   end
...
--       return
--     else
--       delete_scroll_map(api.nvim_get_current_buf())
--       api.nvim_win_close(self.winid, true)
--       self:clean()
--       return
--     end
...
-- vim.api.nvim_create_user_command('DocHover', function(args)
--   hover:render_hover_doc(args)
-- end)

-- vim.keymap.set('n', 'k', '<cmd> DocHover <CR>', { silent = true, nowait = true, desc = 'Pretty Lsp doc hover' })
